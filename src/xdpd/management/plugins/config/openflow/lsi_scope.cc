...
lsi_scope::lsi_scope(std::string name, scope* parent):scope(name, parent, false){
	...
	//Register filters
	register_subscope(new filters_scope(this));
...
void lsi_scope::parse_dpid(libconfig::Setting& setting){
...
void lsi_scope::post_validate(libconfig::Setting& setting, bool dry_run){
	...
	if(!dry_run){
		...
		//Configure filters
		static_cast<filters_scope*>(get_subscope(filters_scope::SCOPE_NAME))->setup_filters(dpid);
	}
}
